...
function stableStringify(value, inArray = false) {
  if (value === undefined) return inArray ? "null" : undefined;
  if (Array.isArray(value)) return `[${value.map((entry) => stableStringify(entry, true)).join(",")}]`;
  ...
  if (typeof value === "number" && (!Number.isFinite(value) || Object.is(value, -0))) throw new AssetForgeError("EXPORT_FAILED", "non-finite glTF number");
...
async function exportCinder(stagingDirectory, version) {
  ...
  try {
    ...
    const importedBounds = new THREE.Box3().setFromObject(imported.scene); const roundTrip = Object.freeze({ schemaVersion: 1, status: "PASS", source: { nodes: canonicalDescription(asset, version).nodes.length, meshes: asset.geometryStatistics.drawCalls, triangles: asset.geometryStatistics.triangles, materials: asset.materialStatistics.materials, bounds: asset.boundingBox, names: canonicalDescription(asset, version).nodes.map(({ name }) => name).sort() }, imported: { nodes, meshes, triangles, materials: materials.size, bounds: { min: importedBounds.min.toArray(), max: importedBounds.max.toArray() }, names: names.sort() }, checks: { hierarchyPresent: nodes === canonicalDescription(asset, version).nodes.length + 1, meshCount: meshes === asset.geometryStatistics.drawCalls, materials: materials.size === asset.materialStatistics.materials, triangles: triangles === asset.geometryStatistics.triangles, bounds: importedBounds.min.distanceTo(new THREE.Vector3(...asset.boundingBox.min)) < 1e-5 && importedBounds.max.distanceTo(new THREE.Vector3(...asset.boundingBox.max)) < 1e-5, finiteTransforms: glbInspection.finiteTransforms, localBuffers: glbInspection.externalUris.length === 0 } });
