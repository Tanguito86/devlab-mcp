...
async function runCapture(browser, server, runName, specBytes, sourceCommit, doLifecycle) {
  ...
  const navigationStarted = performance.now(); await page.goto(`${server.origin}/`, { waitUntil: "load" }); await page.waitForFunction(() => window.__CINDER_PILOT__?.ready === true); const readyMs = performance.now() - navigationStarted;
...
async function main() {
  ...
  const brief = Object.freeze({ assetId: "cinder-relay-drone", function: "industrial military relay drone for devastated zones", silhouette: "compact armored core, broken upper relay arc, two side stabilizers, controlled asymmetry", focus: "ember core and relay arc", noCopy: true });
