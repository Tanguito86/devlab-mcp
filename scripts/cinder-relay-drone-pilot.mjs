...
async function runCapture(browser, server, runName, specBytes, sourceCommit, doLifecycle) {
  ...
  const capturePlan = [...baseCaptures, ...animationCaptures]; const records = []; let captureMs = 0; let firstRenderMs; let maxCapture1024Ms = 0; let rendererMetrics;
  for (const item of capturePlan) {
    const cameraSpecHash = sha(jsonBytes(item.view)); const png = await capturePayload(page, item, "png", specHash, cameraSpecHash, lightingSpecHash); const rgba = await capturePayload(page, item, "raw-rgba", specHash, cameraSpecHash, lightingSpecHash); captureMs += png.captureMs + rgba.captureMs; firstRenderMs ??= png.captureMs; if (item.width === 1024 && item.height === 1024) maxCapture1024Ms = Math.max(maxCapture1024Ms, png.captureMs, rgba.captureMs); rendererMetrics ??= png.rendererMetrics;
  ...
  return { manifest, manifestBytes, records, runtime, rendererMetrics, webgpu, externalUrls, performance: { readyMs, factoryMs: runtime.assetReport.factoryMs, firstRenderMs, maxCapture1024Ms, captureMs, captureCount: records.length }, lifecycle, deviceLoss };
}
...
async function main() {
  ...
  const { factoryMs: _factoryMs, ...canonicalAssetReport } = runA.runtime.assetReport;
  const { cleanupMs, averageCleanupMs, maxCleanupMs, ...canonicalLifecycle } = runA.lifecycle;
  ...
  const resourceReport = Object.freeze({ schema: "devlab-cinder-resource-ownership-v1", ownership: { geometries: 10, materials: 5, textures: 0, renderTargets: 0, all: "OWNED" }, lifecycle: canonicalLifecycle });
  const performanceReport = Object.freeze({ schema: "devlab-cinder-performance-v1", operationalOnly: true, runA: runA.performance, runB: runB.performance, cleanup: { totalMs: cleanupMs, averageMs: averageCleanupMs, maxMs: maxCleanupMs }, firstRender: { calls: webgl.calls, triangles: webgl.triangles }, pngBytes: runA.records.reduce((sum, entry) => sum + entry.byteSize, 0), estimatedPeakMemoryBytes: runA.records.reduce((peak, entry) => Math.max(peak, entry.dimensions.width * entry.dimensions.height * 8), 0), targetsMs: { factory: 500, firstRender: 500, capture1024: 1500, dispose: 100 } });
  ...
  const technicalInput = Object.freeze({ schema: "devlab-cinder-technical-critic-input-v1", assetReport: canonicalAssetReport, determinism, lifecycle: canonicalLifecycle, deviceLoss: runA.deviceLoss, network, noCopy: true, pathSafety: true, typescriptSecurity: true, pngHardening: true, webgl, webgpu: runA.webgpu });
