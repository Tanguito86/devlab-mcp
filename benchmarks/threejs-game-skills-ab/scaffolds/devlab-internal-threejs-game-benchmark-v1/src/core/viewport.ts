export interface ViewportPlan {
  ...
  readonly renderTargetSize: number;
}
...
export function computeViewportPlan(width: number, height: number, devicePixelRatio: number): ViewportPlan {
  if (![width, height, devicePixelRatio].every(Number.isFinite)
    ...
    throw new RangeError("viewport dimensions and pixel ratio must be positive finite numbers");
  }
  ...
  return {
    ...
    aspect: integerWidth / integerHeight,
    pixelRatio: Math.min(devicePixelRatio, 2),
    renderTargetSize: Math.max(64, Math.floor(Math.min(integerWidth, integerHeight) / 3)),
  };
}
