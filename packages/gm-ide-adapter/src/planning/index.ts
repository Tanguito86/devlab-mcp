...
export async function createPlan(projectsDir: string, request: GmPlanRequest, inventory?: ProcessInventory): Promise<GmMutationPlan> {
  ...
  for (const edit of [...request.files].sort((a, b) => a.path.localeCompare(b.path))) {
    const path = safeRelativePath(edit.path); if (!/\.(?:gml|yy|yyp|json)$/i.test(path)) fail("FILE_NOT_ALLOWLISTED", "file extension is not writable", false, { path });
    if (Buffer.byteLength(edit.content, "utf8") > 4 * 1024 * 1024) fail("LIMIT_EXCEEDED", "planned content exceeds per-file limit");
    ...
    const after = Buffer.from(edit.content, "utf8"); files.push(Object.freeze({ path, action: edit.action, beforeSha256: before ? sha256(before) : null, afterSha256: sha256(after), afterContentBase64: after.toString("base64") }));
  }
