...
async function walk(root: string, maxFiles: number): Promise<readonly GmFileSnapshot[]> {
  ...
  }; await visit(root); return Object.freeze(output.sort((a, b) => a.path.localeCompare(b.path)));
}
export async function inspectProject(projectsDir: string, request: GmInspectRequest, inventory?: ProcessInventory): Promise<GmProjectSnapshot> {
  ...
  const base = { schemaVersion: 1 as const, projectRoot, projectType: "GameMaker" as const, projectFile, projectFormat: String((parsed.MetaData as Record<string, unknown> | undefined)?.IDEVersion ?? parsed.resourceVersion ?? "unknown"), files, fileCount: files.length, totalBytes: files.reduce((sum, file) => sum + file.size, 0), gitHead, gitStatus, objects: Object.freeze(refs.filter((path) => path.startsWith("objects/"))), rooms: Object.freeze(refs.filter((path) => path.startsWith("rooms/"))), scripts: Object.freeze(refs.filter((path) => path.startsWith("scripts/"))), references: Object.freeze(refs), processes, warnings: Object.freeze(warnings.sort()), fingerprint };
  const snapshotHash = canonicalHash(base); const snapshot = Object.freeze({ ...base, snapshotHash });
