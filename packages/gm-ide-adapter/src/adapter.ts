...
export class GovernedGameMakerIdeAdapter implements GameMakerIdeAdapter {
  ...
  inspect(request: GmInspectRequest): Promise<GmProjectSnapshot> { return inspectProject(this.projectsDir, request, this.ledger.inventory); }
  plan(request: GmPlanRequest): Promise<GmMutationPlan> { return createPlan(this.projectsDir, request, this.ledger.inventory); }
  applySafe(request: GmApplySafeRequest): Promise<GmApplyResult> { return applyTransaction(this.projectsDir, request, this.ledger.inventory); }
  verify(request: GmVerifyRequest): Promise<GmVerificationResult> { return verifyProject(this.projectsDir, request, this.ledger); }
  rollback(request: GmRollbackRequest): Promise<GmRollbackResult> { return rollbackTransaction(this.projectsDir, request, this.ledger.inventory); }
  async status(request: GmStatusRequest): Promise<GmAdapterStatus> {
    if (request.capability !== "GM_STATUS_V1") fail("GATE_VIOLATION", "status requires GM_STATUS_V1"); const snapshot = await inspectProject(this.projectsDir, { ...request, capability: "GM_STATUS_V1" }, this.ledger.inventory); const transactions = await listTransactions(this.projectsDir, request.evidenceRoot); const ownedIds = new Set(this.ledger.records(request.transactionId).map(({ pid }) => pid)); const ownedProcesses = Object.freeze(snapshot.processes.filter(({ pid }) => ownedIds.has(pid))); const foreignProcesses = Object.freeze(snapshot.processes.filter(({ pid }) => !ownedIds.has(pid)));
